...
# 1. Happy path
def test_submit_to_review_happy_path(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path)
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'submit-to-review', '-r', 'reviewer', '-b', 'main'
    ], capture_output=True, text=True)
    assert result.returncode == 0

# 2. Missing config file
def test_submit_to_review_missing_config_file(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'submit-to-review', '-r', 'reviewer', '-b', 'main'
...
def test_submit_to_review_missing_required_field(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
...
# 4. No current ticket
def test_submit_to_review_no_current_ticket(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
...
def test_submit_to_review_pmt_error(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path, overrides={'in-review': ['INVALID_STATUS']})
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'submit-to-review', '-r', 'reviewer', '-b', 'main'
    ], capture_output=True, text=True)
...
def test_submit_to_review_already_in_target_status(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path)
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
...
def test_submit_to_review_corrupted_config_file(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    with open(config_path, 'w') as f:
...
def test_submit_to_review_env_var_not_set(tmp_path, monkeypatch):
...
def test_submit_to_review_unexpected_argument(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path)
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'submit-to-review', '-r', 'reviewer', '-b', 'main', '--unknown-flag'
    ], capture_output=True, text=True)
...
def test_submit_to_review_multiple_tickets_script(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
