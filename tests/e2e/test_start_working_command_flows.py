...
# 1. Happy path
def test_start_working_happy_path(tmp_path, monkeypatch):
...
def test_start_working_missing_config_file(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'start-working'
...
def test_start_working_missing_required_field(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
...
# 4. No current ticket
def test_start_working_no_current_ticket(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
...
def test_start_working_pmt_error(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path, overrides={'in-progress': ['INVALID_STATUS']})
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'start-working'
    ], capture_output=True, text=True)
...
def test_start_working_already_in_target_status(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path)
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
...
def test_start_working_corrupted_config_file(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    with open(config_path, 'w') as f:
...
def test_start_working_env_var_not_set(tmp_path, monkeypatch):
...
def test_start_working_unexpected_argument(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
    write_config(config_path)
    monkeypatch.setenv('GITASK_CONFIG_PATH', str(config_path))
    result = subprocess.run([
        'gitask', 'start-working', '--unknown-flag'
    ], capture_output=True, text=True)
...
def test_start_working_multiple_tickets_script(tmp_path, monkeypatch):
    config_path = tmp_path / 'config.json'
